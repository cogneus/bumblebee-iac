...
export const addAPI = (
  ...
  const { item, query, template, version, versions } = addAPIRoutes(api);

  const addItem = addAPIFunction(
    scope,
    stackPrefix,
    "add",
    "Adds a template to the service"
  );
  addMethod(addItem, ["POST"], template);

  const getItem = addAPIFunction(
    scope,
    stackPrefix,
    "get",
    "Gets the latest or specific versioned template from the service"
  );
  addMethod(getItem, ["GET"], [...item, ...version]);

  const putItem = addAPIFunction(
    scope,
    stackPrefix,
    "put",
    "Updates a template in the service"
  );
  addMethod(putItem, ["PUT"], item);

  const deleteItem = addAPIFunction(
    scope,
    stackPrefix,
    "remove",
    "Deletes a template in the service"
  );
  addMethod(deleteItem, ["DELETE"], item);

  const getVersions = addAPIFunction(
    scope,
    stackPrefix,
    "versions",
    "Gets versions of a template in the service"
  );
  addMethod(getVersions, ["GET", "POST"], versions);

  const queryItems = addAPIFunction(
    scope,
    stackPrefix,
    "query",
    "Gets template from the service based on a query"
  );
